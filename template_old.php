...
<script>
    ...
    var langData = new Map ([
        ['ru' , {
            ...
            collateralType: "Преобретаемое недвижимое имущество",
        ...
        <div class="row">\
            ...
            <div class="col-sm-6">\
                <div class="form-group form-group-lg">\
                    ...
                    <input type="text" name="collateralTypeId" id="collateralTypeId" class="form-control" value="Полис страхования риска непогашения микрозайма" readonly="">\
                </div>\
            </div>\
        </div>\
        <div class="form-group form-group-lg is-empty">\
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                var date = new Date().toISOString().substr(0, 10);
                    ...
                    var insuranceRisk = parseInt($('#insurance_percent').val()) * parseFloat($('#insuranceRate').val())/1200*parseInt($('#loan_length_input').val())
                    var stateDuty = 0;
                    var others = 0;
                    var additionAmount = 0 + propertyValuation1 + insuranceObject + insuranceRisk + stateDuty + others;
                    $('#pdf').remove();
				//  $('#pdf-box').append('<a id="pdf" style="font-size: 18px; font-weight: bold; color: #222;" href="https://api.kapitalbank.uz/api/v1/pdf/getInformationSheet?amount='+parseInt($('#loan_amount_input').val()).toLocaleString('us', {minimumFractionDigits: 0, maximumFractionDigits: 2}) + ' сум'+'&date='+date.split('-')[2] + '.' + date.split('-')[1] + '.' + date.split('-')[0] +'&mortgage=Микрозайм (Пополнение депозитного счета до востребования или банковской пластиковой карты (или электронного кошелька))&money='+ $('#money').text().toLocaleString('us', {minimumFractionDigits: 0, maximumFractionDigits: 2}) + ' сум'+'&term='+ $('#loan_length_input').val()+'&rate='+$('#loan_percent_input').val()+'%25&cost='+$('#psk').text()+'25&singleAmount='+$('.js-monthly-payment').text() + ' сум'+'&commission=0.00 сум&propertyValuation1=0,00 сум'+'&propertyValuation2=0,00 сум'+'&insuranceObject='+ (parseInt($('#insurance_percent').val())*parseFloat($('#insuranceRate').val())/1200*parseInt($('#loan_length_input').val())).toLocaleString('us', {minimumFractionDigits: 0, maximumFractionDigits: 2})+' сум&stateDuty=0,00 сум'+'&insuranceRisk=0.00 сум&others=0,00 сум&increasedAmount='+parseFloat($('#loan_percent_input').val())*1.5+' процентов годовых&loanSecurity='+loanSec + (parseInt($('#insurance_percent').val().replace(/\s/g, ''))).toLocaleString('us', {minimumFractionDigits: 0, maximumFractionDigits: 2}) + ' сум'+'&additionAmount='+(0 + parseInt(notValid) + parseInt($('#insurance_percent').val())*parseFloat($('#insuranceRate').val())/1200*parseInt($('#loan_length_input').val())+0 + parseInt((parseInt($('#loan_amount_input').val().replace(/\s/g, '')) * 1.25) >= 449000000 ? 337500 : 449000) + 500000).toLocaleString('us', {minimumFractionDigits: 0, maximumFractionDigits: 2}) + ' сум'+'">Скачать информационный лист</a>');
	$('#pdf-box').append('<a id="pdf" style="font-size: 18px; font-weight: bold; color: #222;" href="https://api.kapitalbank.uz/api/v1/pdf/getInformationSheet?amount='+parseInt($('#loan_amount_input').val()).toLocaleString('us', {minimumFractionDigits: 0, maximumFractionDigits: 2}) + ' сум'+'&date='+date.split('-')[2] + '.' + date.split('-')[1] + '.' + date.split('-')[0] +'&mortgage=Использование в торговых точках Партнеров банка, участвующих в Партнерской программе Экспресс кредит 1&money='+ $('#money').text().toLocaleString('us', {minimumFractionDigits: 0, maximumFractionDigits: 2}) + ' сум'+'&term='+ $('#loan_length_input').val()+'&rate='+$('#loan_percent_input').val()+'&fee='+$('#initial_fee_input').val()+'%25&cost='+$('#psk').text()+'25&singleAmount='+$('.js-monthly-payment').text() + ' сум'+'&commission=0.00 сум&propertyValuation1=0,00 сум'+'&propertyValuation2=0,00 сум'+'&insuranceObject=0,00 сум&stateDuty=0,00 сум'+'&additionAmount='+parseFloat($('#insurance_percent').val()) + ' сум&others=0,00 сум&increasedAmount='+parseFloat($('#loan_percent_input').val())*1.5+' процентов годовых&loanSecurity='+loanSec + (1.25*parseInt($('#loan_amount_input').val().replace(/\s/g, ''))).toLocaleString('us', {minimumFractionDigits: 0, maximumFractionDigits: 2}) + ' сум'+'&insuranceRisk='+parseFloat($('#insurance_percent').val()) + ' сум'+'">Скачать информационный лист</a>');
